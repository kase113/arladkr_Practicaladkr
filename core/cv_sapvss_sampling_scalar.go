...
type CVScalarSamplingReport struct {
...
func ResolveCVScalarSampling(
	n, f int, target string, smokeProposerSample, smokeValidatorSample int,
) (CVScalarSamplingReport, error) {
	if n <= 0 || f < 0 || f >= n {
		return CVScalarSamplingReport{}, fmt.Errorf("invalid CV V2 sampling committee")
	}
	...
	var err error
	if normalized != "smoke" {
		normalized, profile, totalBudget, err = cvResolveFailureBudgetScalar(normalized)
		if err != nil {
			return CVScalarSamplingReport{}, err
		}
		if n < 3*f+1 {
			return CVScalarSamplingReport{}, fmt.Errorf("CV V2 secure sampling requires n >= 3f+1")
		}
		perEventTarget = new(big.Rat).Quo(new(big.Rat).Set(totalBudget), big.NewRat(2, 1))
		proposerSample, validatorSample, err = cvScalarExactFinitePopulationSampleSizes(n, f, perEventTarget)
		if err != nil {
			return CVScalarSamplingReport{}, err
		}
	...
	if proposerSample <= 0 || proposerSample > n || validatorSample <= 0 ||
		validatorSample > n {
		return CVScalarSamplingReport{}, fmt.Errorf("invalid CV V2 smoke sampling parameters")
	}
	threshold := validatorSample/2 + 1
	proposer, err := cvScalarProposerFailureBound(n, f, proposerSample)
	if err != nil {
		return CVScalarSamplingReport{}, err
	}
	soundness, liveness, combined, err := cvScalarValidatorFailureBounds(n, f, validatorSample, threshold)
	if err != nil {
		return CVScalarSamplingReport{}, err
	}
	if perEventTarget != nil && (proposer.Cmp(perEventTarget) > 0 || combined.Cmp(perEventTarget) > 0) {
		return CVScalarSamplingReport{}, fmt.Errorf(
	...
	if totalBudget != nil && perEpoch.Cmp(totalBudget) > 0 {
		return CVScalarSamplingReport{}, fmt.Errorf("CV V2 combined sampling bound exceeds total budget %s", normalized)
	}
	totalBudgetText := ""
	perEventTargetText := ""
	if totalBudget != nil {
...
func cvScalarExactFinitePopulationSampleSizes(n, f int, perEventTarget *big.Rat) (int, int, error) {
	...
	for sample := 1; sample <= n; sample++ {
		bound, err := cvScalarProposerFailureBound(n, f, sample)
		if err != nil {
			return 0, 0, err
		}
	...
	for sample := 1; sample <= n; sample++ {
		...
		_, _, combined, err := cvScalarValidatorFailureBounds(n, f, sample, threshold)
		if err != nil {
			return 0, 0, err
		}
...
func CVScalarSamplingUnionBound(report CVScalarSamplingReport, epochs int) (string, error) {
	if epochs < 0 {
...
func cvParseFailureTargetScalar(value string) (*big.Rat, error) {
...
func cvResolveFailureBudgetScalar(value string) (target, profile string, budget *big.Rat, err error) {
	value = strings.ToLower(strings.TrimSpace(value))
	switch value {
	case "original":
	...
	default:
		parsed, parseErr := cvParseFailureTargetScalar(value)
		if parseErr != nil {
			return "", "", nil, parseErr
		}
