...
func cvComponentPipelineTestRefsScalar(count int) []cvComponentRefScalar {
	refs := make([]cvComponentRefScalar, count)
...
func TestCVComponentPipelineVerifiesBeforeAllRecoveryCompletes(t *testing.T) {
	refs := cvComponentPipelineTestRefsScalar(8)
	releaseRecovery := make(chan struct{})
	verificationStarted := make(chan struct{})
	var signalOnce sync.Once
	done := make(chan []cvComponentVerificationResultScalar, 1)

	go func() {
		results, _ := cvRunComponentPipelineScalar(
			refs, 4, 4,
			func(ref cvComponentRefScalar) cvComponentVerificationResultScalar {
				if ref.Header.DealerID != 0 {
					<-releaseRecovery
				}
				return cvComponentVerificationResultScalar{ref: ref}
			},
...
func TestCVComponentPipelineBoundsVerifierConcurrency(t *testing.T) {
	refs := cvComponentPipelineTestRefsScalar(12)
