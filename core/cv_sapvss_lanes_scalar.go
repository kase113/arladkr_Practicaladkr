...
const cvOwnershipProofChallengeScalar = "ARL-CV-sAPVSS/v2-scalar-group/ownership-proof/challenge"

type cvOwnershipProofScalar struct {
...
type cvReceiverLaneOfferScalar struct {
	ReceiverID    int
	ReceiverIndex int
...
type cvDealerReceiverWitnessScalar struct {
	ScalarDigits []uint64
...
// cvReceiverLaneRandomnessScalar separates fresh randomness from the group math so
...
type cvReceiverLaneRandomnessScalar struct {
	scalarCoins        []fr.Element
	blindingCoin       fr.Element
...
func cvGenerateReceiverLaneRandomnessScalar(chunks int) (*cvReceiverLaneRandomnessScalar, error) {
	if chunks <= 0 {
		return nil, fmt.Errorf("invalid CV V2 receiver lane randomness dimensions")
	}
	randomness := &cvReceiverLaneRandomnessScalar{
...
func cvEncryptReceiverLanesScalar(
	context *cvLeafContextScalar, dealerID, receiverID, receiverIndex int,
	receiverPublicKey *bls12381.G1Affine, scalar, blinding fr.Element,
) (*cvReceiverLaneOfferScalar, *cvDealerReceiverWitnessScalar, error) {
	if err := cvValidateReceiverBindingScalar(context, receiverID, receiverIndex, receiverPublicKey); err != nil ||
		dealerID < 0 {
	...
	randomness, err := cvGenerateReceiverLaneRandomnessScalar(chunks)
	if err != nil {
		return nil, nil, err
	}
	return cvComputeReceiverLaneOfferScalar(
		context, dealerID, receiverID, receiverIndex, receiverPublicKey, scalar, blinding, randomness,
	)
}

// cvComputeReceiverLaneOfferScalar builds a lane offer using batched fixed-base multiplication.
func cvComputeReceiverLaneOfferScalar(
	context *cvLeafContextScalar, dealerID, receiverID, receiverIndex int,
	receiverPublicKey *bls12381.G1Affine, scalar, blinding fr.Element,
	randomness *cvReceiverLaneRandomnessScalar,
) (*cvReceiverLaneOfferScalar, *cvDealerReceiverWitnessScalar, error) {
	...
	weightedDigitNonce, err := cvWeightedScalarScalar(randomness.digitNonces, context.Profile.chunkBits)
	if err != nil {
		return nil, nil, err
	}
	...
	witness := &cvDealerReceiverWitnessScalar{
	...
	if err := cvFinishOwnershipProofScalar(
...
func cvProveOwnershipScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, witness *cvDealerReceiverWitnessScalar,
) (*cvOwnershipProofScalar, error) {
	return cvProveOwnershipModeScalar(context, dealerID, offer, receiverPublicKey, witness, true)
}

func cvProveOwnershipModeScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, witness *cvDealerReceiverWitnessScalar, validateLane bool,
) (*cvOwnershipProofScalar, error) {
	var shapeErr error
	if validateLane {
		shapeErr = cvValidateLaneOfferShapeScalar(context, offer, receiverPublicKey)
	} else {
		shapeErr = cvValidateLaneOfferShapeAfterPointDecodingScalar(context, offer, receiverPublicKey)
	}
	...
	weightedDigitNonce, err := cvWeightedScalarScalar(digitNonces, context.Profile.chunkBits)
	if err != nil {
		return nil, err
	}
...
// cvFinishOwnershipProofScalar completes the Fiat-Shamir ownership proof.
func cvFinishOwnershipProofScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, witness *cvDealerReceiverWitnessScalar,
	proof *cvOwnershipProofScalar, coinNonces, digitNonces []fr.Element,
	...
	challenge, err := cvOwnershipChallengeScalarAfterValidationScalar(
...
func cvEncryptPointAfterValidationScalar(
...
func cvVerifyOwnershipScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine,
) error {
	return cvVerifyOwnershipModeScalar(context, dealerID, offer, receiverPublicKey, true)
}

func cvVerifyOwnershipAfterPointDecodingScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine,
) error {
	return cvVerifyOwnershipModeScalar(context, dealerID, offer, receiverPublicKey, false)
}

func cvVerifyOwnershipModeScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	...
	weightedResponse, err := cvWeightedScalarScalar(proof.ScalarDigitResponses, context.Profile.chunkBits)
	if err != nil {
		return err
	}
...
func cvVerifyAndDecryptReceiverLanesModeScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, receiverSecret fr.Element, validatePoints bool,
) (fr.Element, bls12381.G1Affine, error) {
	// Verify ownership before using the receiver secret.
	var ownershipErr error
	if validatePoints {
		ownershipErr = cvVerifyOwnershipBatchScalar(
			context, dealerID, []*cvReceiverLaneOfferScalar{offer},
	...
	solver := cvNewBoundedDLogSolverForBaseScalar(&genG1, base-1)
	for chunk := 0; chunk < chunks; chunk++ {
		plaintext := cvDecryptGroupCiphertextScalar(&offer.ScalarChunks[chunk], receiverSecret)
	...
	scalar, err := cvDigitsToScalarScalar(digits, context.Profile.chunkBits)
	if err != nil {
		return fr.Element{}, bls12381.G1Affine{}, err
	}
	blindingOpening := cvDecryptGroupCiphertextScalar(&offer.Blinding, receiverSecret)
...
func cvDecryptGroupCiphertextScalar(ciphertext *cvElGamalCiphertext, secret fr.Element) bls12381.G1Affine {
...
func cvValidateReceiverBindingScalar(
	context *cvLeafContextScalar, receiverID, receiverIndex int, receiverPublicKey *bls12381.G1Affine,
) error {
	if err := cvValidateLeafContextScalar(context); err != nil || receiverIndex <= 0 ||
...
func cvValidateLaneOfferShapeScalar(
	context *cvLeafContextScalar, offer *cvReceiverLaneOfferScalar, receiverPublicKey *bls12381.G1Affine,
) error {
	if offer == nil || cvValidateReceiverBindingScalar(context, offer.ReceiverID, offer.ReceiverIndex, receiverPublicKey) != nil ||
...
func cvValidateLaneOfferShapeAfterPointDecodingScalar(
	context *cvLeafContextScalar, offer *cvReceiverLaneOfferScalar, receiverPublicKey *bls12381.G1Affine,
) error {
	if err := cvValidateLeafContextScalar(context); err != nil || offer == nil || receiverPublicKey == nil ||
...
func cvOwnershipChallengeScalarAfterValidationScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, proof *cvOwnershipProofScalar,
) (fr.Element, error) {
	...
	return cvOwnershipChallengeScalarModeScalar(context, dealerID, offer, receiverPublicKey, proof, false)
}

func cvOwnershipChallengeScalarModeScalar(
	context *cvLeafContextScalar, dealerID int, offer *cvReceiverLaneOfferScalar,
	receiverPublicKey *bls12381.G1Affine, proof *cvOwnershipProofScalar, validateProofPoints bool,
) (fr.Element, error) {
	...
	return cvHashToFr(cvOwnershipProofChallengeScalar, wire.Bytes())
}

func cvWeightedScalarScalar(values []fr.Element, chunkBits uint) (fr.Element, error) {
...
type cvBoundedDLogSolverScalar struct {
	bound         uint64
	width         uint64
...
func cvNewBoundedDLogSolverForBaseScalar(base *bls12381.G1Affine, bound uint64) *cvBoundedDLogSolverScalar {
	rangeSize := bound + 1
	width := uint64(math.Ceil(math.Sqrt(float64(rangeSize))))
	for width*width < rangeSize {
		width++
	}
	solver := &cvBoundedDLogSolverScalar{
...
func (solver *cvBoundedDLogSolverScalar) solve(target *bls12381.G1Affine) (uint64, bool) {
...
func cvDigitsToScalarScalar(digits []uint64, chunkBits uint) (fr.Element, error) {
