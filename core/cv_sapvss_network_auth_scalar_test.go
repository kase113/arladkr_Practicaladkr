...
func cvNetworkAuthScalarFixture(t *testing.T) (*cvNetworkAuthenticatorScalar, *cvValidatorKeyMaterialScalar, *cvReceiverKeyMaterialScalar, Config) {
	t.Helper()
...
func TestCVNetworkAuthenticationScalarSeparatesBLSAndEd25519Roles(t *testing.T) {
	...
	tests := []struct {
		...
		{from: cfg.OldCommittee[0], to: cfg.NewCommittee[0], tag: cvTagHandoffScalar},
		{from: cfg.OldCommittee[1], to: cfg.NewCommittee[0], tag: cvTagAggregateRecoverStoreScalar},
		{from: cfg.OldCommittee[1], to: cfg.NewCommittee[0], tag: cvTagAggregatePayloadScalar},
		{from: cfg.NewCommittee[0], to: cfg.OldCommittee[0], tag: cvTagAggregateRecoverGetScalar},
		{from: cfg.NewCommittee[0], to: cfg.OldCommittee[0], tag: cvTagAggregateRecoverCancelScalar},
		{from: cfg.NewCommittee[0], to: cfg.OldCommittee[0], tag: cvTagAggregatePayloadGetScalar},
...
func TestCVNetworkAuthenticationScalarDoesNotUseReceiverEncryptionSecret(t *testing.T) {
	_, validators, receivers, cfg := cvNetworkAuthScalarFixture(t)
	withoutIdentity := *receivers
	withoutIdentity.localIdentitySecrets = nil
	auth, err := newCVNetworkAuthenticatorScalar(validators, &withoutIdentity)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := auth.seal(receiver, cfg.OldCommittee[0], cvTagAggregateRecoverGetScalar, envelope); err == nil {
		t.Fatal("receiver encryption secret authenticated a V2 recovery request")
	}
}

func TestCVSAPVSSRouterAuthenticatesScalarHandoffAndRecovery(t *testing.T) {
	auth, _, _, cfg := cvNetworkAuthScalarFixture(t)
	...
	handoffWire, err := auth.seal(oldNode, newNode, cvTagHandoffScalar, envelope)
	if err != nil {
		t.Fatal(err)
	}
	requestWire, err := auth.seal(newNode, oldNode, cvTagAggregateRecoverGetScalar, envelope)
	if err != nil {
		t.Fatal(err)
	}
	transport.inject(newNode, Message{From: oldNode, To: newNode, Tag: cvTagHandoffScalar, Body: handoffWire})
	transport.inject(oldNode, Message{From: newNode, To: oldNode, Tag: cvTagAggregateRecoverGetScalar, Body: requestWire})
	transport.inject(newNode, Message{From: oldNode, To: newNode, Tag: cvTagHandoffScalar, Body: envelope})
