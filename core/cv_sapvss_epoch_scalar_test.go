...
func TestCVRecoverServiceGraceScalarBounds(t *testing.T) {
	tests := []struct {
		route time.Duration
		want  time.Duration
	}{
	...
	for _, test := range tests {
		if got := cvRecoverServiceGraceScalar(test.route); got != test.want {
			t.Fatalf("route=%v grace=%v want %v", test.route, got, test.want)
		}
	}
}

func TestRunCVEpochScalarFourNodeEndToEnd(t *testing.T) {
	if testing.Short() {
	...
	base := Config{
		...
		AgreementBasePort: cvAvailableEpochScalarBasePort(t),
		CVPublicKeyDir:    publicDir, CVLocalSecretDir: secretDir,
		WaitSPBCTimeout: 30 * time.Second, RouteSendTimeout: time.Second,
	}
	if err := GenerateCVScalarKeyMaterial(publicDir, secretDir, base); err != nil {
		t.Fatal(err)
	}
	...
	if len(failures) != 0 {
		...
		t.Fatalf("CV V2 epoch failed on %d nodes (coin shares sent=%d)", len(failures), transport.sentCount(cvTagCoinShareScalar))
	}
	if senders := transport.sentFromByTag(cvTagPoolOfferScalar); len(senders) == 0 || len(senders) > base.CVProposerSampleSize {
		t.Fatalf("sampled proposer Pool senders=%v want between 1 and %d", senders, base.CVProposerSampleSize)
	}
	if senders := transport.sentFromByTag(cvTagValidationRequestScalar); len(senders) == 0 || len(senders) > base.CVProposerSampleSize {
	...
	for node, result := range got {
		...
		if result.CVAPVSSMode != cvSAPVSSScalarProtocolVersion || result.AgreementMode != "single-mvba-v2" {
...
func cvAvailableEpochScalarBasePort(t *testing.T) int {
	t.Helper()
