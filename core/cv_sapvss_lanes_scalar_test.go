...
func cvReceiverLanesScalarFixture(
	t *testing.T,
) (*cvLeafContextScalar, int, int, int, fr.Element, bls12381.G1Affine, fr.Element, fr.Element) {
	t.Helper()
	context, coefficients, blindings := cvCoreProofScalarFixture(t)
...
func TestCVReceiverLanesScalarUseScalarChunksAndGroupBlinding(t *testing.T) {
	context, dealer, receiverID, receiverIndex, secret, publicKey, scalar, blinding := cvReceiverLanesScalarFixture(t)
	...
	if err := cvVerifyOwnershipScalar(context, dealer, offer, &publicKey); err != nil {
		t.Fatalf("verify V2 ownership proof: %v", err)
	}
	recoveredScalar, recoveredBlinding, err := cvVerifyAndDecryptReceiverLanesScalar(
...
func TestCVReceiverLanesScalarDecodedPathStillVerifiesOwnershipEquations(t *testing.T) {
	context, dealer, receiverID, receiverIndex, secret, publicKey, scalar, blinding := cvReceiverLanesScalarFixture(t)
	...
	wire, err := cvReceiverLaneOfferScalarCanonicalBytesAfterValidation(context, dealer, offer)
	if err != nil {
		t.Fatal(err)
	}
	decoded, err := cvDecodeReceiverLaneOfferBeforeVerificationScalar(
		wire, context, dealer, receiverID, receiverIndex, &publicKey,
	)
	if err != nil {
		t.Fatal(err)
	}
	if _, _, err := cvVerifyAndDecryptReceiverLanesAfterPointDecodingScalar(
	...
	badProof := *decoded
	badProof.Ownership = cvCloneOwnershipProofScalar(&decoded.Ownership)
	one := fr.One()
...
func TestCVOwnershipProofScalarRejectsCiphertextEvaluationAndBindingMutations(t *testing.T) {
	...
	badCiphertext.ScalarChunks[0].c.Add(&badCiphertext.ScalarChunks[0].c, &genG1)
	if err := cvVerifyOwnershipScalar(context, dealer, &badCiphertext, &publicKey); err == nil {
		t.Fatal("accepted ownership proof after ciphertext mutation")
	}
	badBlinding := *offer
	badBlinding.Blinding.c.Add(&badBlinding.Blinding.c, &genG1)
	if err := cvVerifyOwnershipScalar(context, dealer, &badBlinding, &publicKey); err == nil {
	...
	if err := cvVerifyOwnershipScalar(context, dealer, &badEvaluation, &publicKey); err == nil {
		t.Fatal("accepted ownership proof after evaluation mutation")
	}
	if err := cvVerifyOwnershipScalar(context, dealer+1, offer, &publicKey); err == nil {
		t.Fatal("accepted ownership proof under another dealer")
	}
	badReceiver := *offer
	badReceiver.ReceiverID = context.NewRoster[0]
	if err := cvVerifyOwnershipScalar(context, dealer, &badReceiver, &publicKey); err == nil {
		t.Fatal("accepted ownership proof under a mismatched receiver binding")
	}
	badIndex := *offer
	badIndex.ReceiverIndex = 1
	if err := cvVerifyOwnershipScalar(context, dealer, &badIndex, &publicKey); err == nil {
	...
	if err := cvVerifyOwnershipScalar(&badContext, dealer, offer, &publicKey); err == nil {
		t.Fatal("accepted ownership proof under another receiver registry")
	}
	badProof := *offer
	badProof.Ownership = cvCloneOwnershipProofScalar(&offer.Ownership)
	var one fr.Element
	one.SetOne()
	badProof.Ownership.BlindingShareResponse.Add(&badProof.Ownership.BlindingShareResponse, &one)
	if err := cvVerifyOwnershipScalar(context, dealer, &badProof, &publicKey); err == nil {
	...
	if _, _, err := cvVerifyAndDecryptReceiverLanesScalar(context, dealer, offer, &publicKey, wrongSecret); err == nil {
		t.Fatal("decrypted V2 lanes with a secret outside the receiver registry")
	}
}

func TestCVReceiverLanesScalarRejectOutOfRangeDigitAfterValidOwnershipProof(t *testing.T) {
	context, dealer, receiverID, receiverIndex, secret, publicKey, scalar, blinding := cvReceiverLanesScalarFixture(t)
	offer, witness, err := cvEncryptReceiverLanesScalar(
		context, dealer, receiverID, receiverIndex, &publicKey, scalar, blinding,
	)
	...
	proof, err := cvProveOwnershipScalar(context, dealer, offer, &publicKey, witness)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := cvVerifyAndDecryptReceiverLanesScalar(context, dealer, offer, &publicKey, secret); err == nil {
		t.Fatal("accepted out-of-range V2 digit after valid ownership proof")
	}
}

func TestCVReceiverLanesScalarBatchedOfferMatchesScalarReference(t *testing.T) {
	context, dealer, receiverID, receiverIndex, secret, publicKey, scalar, blinding := cvReceiverLanesScalarFixture(t)
	chunks, err := cvChunkCount(context.Profile)
	if err != nil {
		t.Fatal(err)
	...
	offer, witness, err := cvComputeReceiverLaneOfferScalar(
...
// cvEncryptReceiverLanesLegacyScalar preserves the pre-batching implementation for
// equivalence and performance comparison against cvComputeReceiverLaneOfferScalar.
func cvEncryptReceiverLanesLegacyScalar(
	...
	for chunk, digit := range witness.ScalarDigits {
		...
		offer.ScalarChunks[chunk] = cvEncryptPointAfterValidationScalar(
	...
	offer.Blinding = cvEncryptPointAfterValidationScalar(receiverPublicKey, &blindingPoint, witness.BlindingCoin)
...
func TestCVReceiverLanesScalarBatchedMatchesLegacyImplementation(t *testing.T) {
	context, dealer, receiverID, receiverIndex, _, publicKey, scalar, blinding := cvReceiverLanesScalarFixture(t)
	chunks, err := cvChunkCount(context.Profile)
	if err != nil {
		t.Fatal(err)
	...
	batched, batchedWitness, err := cvComputeReceiverLaneOfferScalar(
		context, dealer, receiverID, receiverIndex, &publicKey, scalar, blinding, randomness,
	)
	if err != nil {
		t.Fatal(err)
	}
	legacy, legacyWitness, err := cvEncryptReceiverLanesLegacyScalar(
		context, dealer, receiverID, receiverIndex, &publicKey, scalar, blinding, randomness,
	)
	if err != nil {
		t.Fatal(err)
	}
	batchedWire, err := cvReceiverLaneOfferScalarCanonicalBytesAfterValidation(context, dealer, batched)
	if err != nil {
		t.Fatal(err)
	}
	legacyWire, err := cvReceiverLaneOfferScalarCanonicalBytesAfterValidation(context, dealer, legacy)
	if err != nil {
		t.Fatal(err)
	}
