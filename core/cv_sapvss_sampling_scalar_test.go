...
func TestResolveCVScalarSamplingMatchesPaperProfiles(t *testing.T) {
	tests := []struct {
		profile                     string
		n, f                        int
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("%s-n%d", test.profile, test.n), func(t *testing.T) {
			report, err := ResolveCVScalarSampling(test.n, test.f, test.profile, 3, 3)
			if err != nil {
				t.Fatal(err)
			}
...
func TestResolveCVScalarSamplingUsesTotalBudgetAndMinimalSamples(t *testing.T) {
	report, err := ResolveCVScalarSampling(128, 42, "1e-10", 3, 3)
	if err != nil {
		t.Fatal(err)
	}
	...
	previousProposer, err := cvScalarProposerFailureBound(128, 42, report.ProposerSampleSize-1)
	...
	_, _, previousValidatorBound, err := cvScalarValidatorFailureBounds(
...
func TestResolveCVScalarSamplingLabelsExplicitSmoke(t *testing.T) {
	report, err := ResolveCVScalarSampling(7, 2, "smoke", 3, 2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestResolveCVScalarSamplingRejectsUnsupportedTarget(t *testing.T) {
	for _, target := range []string{"0.01", "1e-0", "2^-0", "paper", "unknown"} {
		if _, err := ResolveCVScalarSampling(128, 42, target, 3, 3); err == nil {
			t.Fatalf("accepted unsupported target %q", target)
		}
	}
}

func TestCVScalarSamplingUnionBoundIsExactAndCapped(t *testing.T) {
	report, err := ResolveCVScalarSampling(7, 2, "smoke", 3, 3)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got, err := CVScalarSamplingUnionBound(report, 3); err != nil || got != "3/7" {
		t.Fatalf("three-epoch union bound=%q err=%v", got, err)
	}
	if got, err := CVScalarSamplingUnionBound(report, 8); err != nil || got != "1" {
