...
const cvAPDBStoredShareWireDomainScalar = "ARL-CV-sAPVSS/v2-scalar-group/apdb-stored-share"

type cvAPDBStoredShareScalar struct {
	InstanceDigest []byte
	Root           []byte
	Share          []byte
}

type cvAPDBLockCollectorScalar struct {
	mu          sync.Mutex
...
func newCVAPDBLockCollectorScalar(
	encoded *cvAPDBEncodedScalar, oldRoster []int, apdbSigner *tblsThresholdSigner,
) (*cvAPDBLockCollectorScalar, error) {
	if encoded == nil || len(encoded.instanceDigest) != 32 || len(encoded.root) != 32 ||
		...
		!equalInts(oldRoster, sortedUnique(oldRoster)) || !cvScalarSignerHasRole(apdbSigner, cvScalarRoleAPDB) ||
	...
	encodedCopy := &cvAPDBEncodedScalar{
		...
		stores:         make([]cvAPDBStoreScalar, len(encoded.stores)),
	}
	for index, member := range oldRoster {
		...
		if member < 0 || store.Index != index || !bytes.Equal(store.InstanceDigest, encoded.instanceDigest) ||
			...
			cvVerifyAPDBStoreScalar(store, encoded.totalShards, encoded.shardBytes) != nil {
			return nil, fmt.Errorf("invalid CV V2 APDB lock collector roster")
		}
		memberIndex[member] = index
		encodedCopy.stores[index] = cvAPDBStoreScalar{
	...
	statement, err := cvAPDBStoredStatementScalar(encoded.instanceDigest, encoded.root)
	if err != nil {
		return nil, err
	}
	return &cvAPDBLockCollectorScalar{
...
func (c *cvAPDBLockCollectorScalar) StoreRecipients() []int {
	if c == nil {
		return nil
	}
...
func (c *cvAPDBLockCollectorScalar) StoreOffer(holder int) ([]byte, error) {
	if c == nil {
	...
	return cvAPDBStoreScalarCanonicalBytes(&c.encoded.stores[index], c.encoded.totalShards, c.encoded.shardBytes)
}

func (c *cvAPDBLockCollectorScalar) AddStoredShare(from int, wire []byte) (bool, error) {
	if c == nil {
	...
	response, err := cvDecodeAPDBStoredShareScalar(wire)
	if err != nil {
		return false, err
	}
...
func (c *cvAPDBLockCollectorScalar) AddDecodedStoredShare(from int, response *cvAPDBStoredShareScalar, wire []byte) (bool, error) {
...
func (c *cvAPDBLockCollectorScalar) RecoverLock() (*cvAPDBLockScalar, error) {
	if c == nil {
		return nil, fmt.Errorf("nil CV V2 APDB lock collector")
	}
	c.mu.Lock()
	...
	lock, err := cvNewAPDBLockScalar(c.encoded, certificate)
	if err != nil {
		return nil, err
	}
	if err := cvVerifyAPDBLockScalar(lock, c.signer); err != nil {
		return nil, err
	}
	return lock, nil
}

func cvHandleAPDBStoreOfferScalar(
	sid string, epoch uint64, proposer, holder int, oldRoster []int, offerWire []byte,
	totalShards, shardBytes int, holderStore *cvAPDBHolderStoreScalar, apdbSigner *tblsThresholdSigner,
) ([]byte, error) {
	if !equalInts(oldRoster, sortedUnique(oldRoster)) || !cvMemberInRosterScalar(proposer, oldRoster) ||
		!cvMemberInRosterScalar(holder, oldRoster) || holderStore == nil {
		return nil, fmt.Errorf("invalid CV V2 APDB store offer participants")
	}
	store, err := cvDecodeAPDBStoreScalar(offerWire, totalShards, shardBytes)
	if err != nil {
		return nil, err
	}
	...
	return cvAPDBStoredShareScalarCanonicalBytes(&cvAPDBStoredShareScalar{
		InstanceDigest: store.InstanceDigest, Root: store.Root, Share: share,
	})
}

func cvHandleAPDBRecoveryRequestScalar(
	sid string, epoch uint64, requester, holder int, oldRoster []int, requestWire []byte,
	totalShards, shardBytes int, holderStore *cvAPDBHolderStoreScalar, apdbSigner *tblsThresholdSigner,
) ([]byte, error) {
	if !equalInts(oldRoster, sortedUnique(oldRoster)) || !cvMemberInRosterScalar(requester, oldRoster) ||
		!cvMemberInRosterScalar(holder, oldRoster) || holderStore == nil {
		return nil, fmt.Errorf("invalid CV V2 APDB recovery participants")
	}
	lock, err := cvDecodeAPDBLockScalar(requestWire)
	if err != nil || cvVerifyAPDBLockScalar(lock, apdbSigner) != nil {
		return nil, fmt.Errorf("invalid CV V2 APDB recovery lock")
	}
	return cvHandleAPDBRecoveryLockScalar(sid, epoch, requester, holder, oldRoster, lock,
		totalShards, shardBytes, holderStore)
}

func cvHandleAPDBRecoveryLockScalar(
	sid string, epoch uint64, requester, holder int, oldRoster []int, lock *cvAPDBLockScalar,
	totalShards, shardBytes int, holderStore *cvAPDBHolderStoreScalar,
) ([]byte, error) {
	if !equalInts(oldRoster, sortedUnique(oldRoster)) || !cvMemberInRosterScalar(requester, oldRoster) ||
		!cvMemberInRosterScalar(holder, oldRoster) || holderStore == nil || lock == nil {
		return nil, fmt.Errorf("invalid CV V2 APDB recovery participants")
	}
	...
	return cvAPDBStoreScalarCanonicalBytes(store, totalShards, shardBytes)
}

func cvHandleAggregateRecoveryRequestScalar(
	sid string, epoch uint64, receiver, holder int, oldRoster, newRoster []int, requestWire, expectedContext []byte,
	totalShards, shardBytes int, holderStore *cvAPDBHolderStoreScalar, apdbSigner, controlSigner *tblsThresholdSigner,
) ([]byte, error) {
	if !equalInts(oldRoster, sortedUnique(oldRoster)) || !equalInts(newRoster, sortedUnique(newRoster)) ||
		!cvMemberInRosterScalar(receiver, newRoster) || !cvMemberInRosterScalar(holder, oldRoster) || holderStore == nil {
...
func cvAPDBStoredShareScalarCanonicalBytes(response *cvAPDBStoredShareScalar) ([]byte, error) {
	...
	_ = cvWriteBytes(&wire, []byte(cvAPDBStoredShareWireDomainScalar))
...
func cvDecodeAPDBStoredShareScalar(wire []byte) (*cvAPDBStoredShareScalar, error) {
	r := newCVWireReader(wire)
	domain, err := r.bytes(len(cvAPDBStoredShareWireDomainScalar))
	if err != nil || !bytes.Equal(domain, []byte(cvAPDBStoredShareWireDomainScalar)) {
	...
	response := &cvAPDBStoredShareScalar{InstanceDigest: instance, Root: root, Share: share}
	canonical, err := cvAPDBStoredShareScalarCanonicalBytes(response)
...
func cvMemberInRosterScalar(member int, roster []int) bool {
