...
func TestCVAPDBServiceScalarLockPDAndComponentRecovery(t *testing.T) {
	_, public := cvAgreementObjectScalarFixture(t)
	payload := []byte("network-service APDB LockPD and validated recovery")
	instance, err := cvAPDBInstanceDigestScalar("COMP", []byte("service round trip"))
	if err != nil {
		t.Fatal(err)
	}
	encoded, err := cvAPDBEncodeScalar(instance, payload, public.Params.recoveryThreshold, len(public.OldCommittee), 2048)
	if err != nil {
		t.Fatal(err)
	}
	lockCollector, err := newCVAPDBLockCollectorScalar(encoded, public.OldCommittee, public.APDBSigner)
	if err != nil {
		t.Fatal(err)
	}
	...
	for index, holder := range public.OldCommittee[:public.APDBSigner.Threshold()] {
		...
		response, err := cvHandleAPDBStoreOfferScalar(public.SID, public.Epoch, proposer, holder, public.OldCommittee,
	...
	if err != nil || cvVerifyAPDBLockScalar(lock, public.APDBSigner) != nil {
	...
	recovery, err := newCVAPDBRecoveryCollectorScalar(lock, public.OldCommittee, public.Params.recoveryThreshold,
		encoded.shardBytes, 2048, public.APDBSigner, nil)
	if err != nil {
		t.Fatal(err)
	}
	for index, holder := range public.OldCommittee[:public.Params.recoveryThreshold] {
		response, err := cvHandleAPDBRecoveryRequestScalar(public.SID, public.Epoch, proposer, holder,
...
func TestCVAPDBServiceScalarRejectsShareReplayAndUnauthorizedParticipants(t *testing.T) {
	_, public := cvAgreementObjectScalarFixture(t)
	instance, err := cvAPDBInstanceDigestScalar("COMP", []byte("service mutations"))
	if err != nil {
		t.Fatal(err)
	}
	encoded, err := cvAPDBEncodeScalar(instance, []byte("service mutation payload"), public.Params.recoveryThreshold,
		len(public.OldCommittee), 1024)
	if err != nil {
		t.Fatal(err)
	}
	collector, err := newCVAPDBLockCollectorScalar(encoded, public.OldCommittee, public.APDBSigner)
	if err != nil {
		t.Fatal(err)
	}
	holderStore, err := newCVAPDBHolderStoreScalar(t.TempDir())
	...
	response, err := cvHandleAPDBStoreOfferScalar(public.SID, public.Epoch, public.OldCommittee[1], holder,
	...
	if _, err := cvHandleAPDBStoreOfferScalar(public.SID, public.Epoch, 9999, holder, public.OldCommittee,
	...
	if _, err := cvHandleAPDBRecoveryRequestScalar(public.SID, public.Epoch, 9999, holder, public.OldCommittee,
