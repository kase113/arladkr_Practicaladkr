...
const (
	cvFallbackLinkWireDomainScalar      = "ARL-CV-sAPVSS/v2-scalar-group/fallback-link"
	cvFallbackLinkChallengeDomainScalar = "ARL-CV-sAPVSS/v2-scalar-group/fallback-link/challenge"
)

type cvFallbackLinkProofScalar struct {
...
// cvFallbackDigitWitnessScalar stays local and supplies the exact Pedersen
// blindings consumed by the aggregated scalar-digit range proof.
type cvFallbackDigitWitnessScalar struct {
	Blindings []fr.Element
}

func cvProveFallbackLinkScalar(
...
) (*cvFallbackLinkProofScalar, *cvFallbackDigitWitnessScalar, error) {
	...
	proof := &cvFallbackLinkProofScalar{
	...
	local := &cvFallbackDigitWitnessScalar{Blindings: make([]fr.Element, total)}
	...
	for receiver, witness := range witnesses {
		...
		weighted, err := cvFallbackWeightedNoncesScalar(receiver, chunks, context.Profile.chunkBits, digitNonces)
		if err != nil {
			return nil, nil, err
		}
	...
	challenge, err := cvFallbackLinkChallengeAfterValidationScalar(
...
func cvVerifyFallbackLinkScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine, proof *cvFallbackLinkProofScalar,
) error {
	return cvVerifyFallbackLinkModeScalar(context, dealerID, offers, receiverPublicKeys, proof, true)
}

func cvVerifyFallbackLinkAfterPointDecodingScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine, proof *cvFallbackLinkProofScalar,
) error {
	return cvVerifyFallbackLinkModeScalar(context, dealerID, offers, receiverPublicKeys, proof, false)
}

func cvVerifyFallbackLinkModeScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine, proof *cvFallbackLinkProofScalar, validatePoints bool,
) error {
	...
	if err != nil || !cvValidFallbackLinkProofShapeModeScalar(proof, total, len(offers), validatePoints) {
	...
	for receiver, offer := range offers {
		...
		weighted, err := cvWeightedScalarScalar(responses, context.Profile.chunkBits)
		if err != nil {
			return err
		}
...
func cvValidateFallbackLinkStatementAfterPointDecodingScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine,
) (int, int, error) {
	if err := cvValidateLeafContextScalar(context); err != nil || dealerID < 0 ||
		!cvMemberInRosterScalar(dealerID, context.OldRoster) || len(offers) == 0 ||
		len(offers) != len(receiverPublicKeys) || len(offers) > cvNewFaultBoundFromContextScalar(context) {
	...
	for i, offer := range offers {
		if offer == nil || receiverPublicKeys[i].IsInfinity() || offer.ReceiverIndex <= previous ||
			cvValidateLaneOfferShapeAfterPointDecodingScalar(context, offer, &receiverPublicKeys[i]) != nil {
...
func cvFallbackLinkChallengeAfterValidationScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine, proof *cvFallbackLinkProofScalar,
) (fr.Element, error) {
	...
	return cvFallbackLinkChallengeModeScalar(context, dealerID, offers, receiverPublicKeys, proof, false)
}

func cvFallbackLinkChallengeModeScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine, proof *cvFallbackLinkProofScalar, validatePoints bool,
) (fr.Element, error) {
	...
	for _, points := range cvFallbackLinkPointVectorsScalar(proof) {
		if err := cvWritePointVectorMode(&wire, points, validatePoints); err != nil {
			return fr.Element{}, err
		}
	}
	return cvHashToFr(cvFallbackLinkChallengeDomainScalar, wire.Bytes())
}

func cvValidateFallbackLinkStatementScalar(
	context *cvLeafContextScalar, dealerID int, offers []*cvReceiverLaneOfferScalar,
	receiverPublicKeys []bls12381.G1Affine,
) (int, int, error) {
	if err := cvValidateLeafContextScalar(context); err != nil || dealerID < 0 ||
	...
	for i, offer := range offers {
		if offer == nil || offer.ReceiverIndex <= previous ||
			cvValidateLaneOfferShapeScalar(context, offer, &receiverPublicKeys[i]) != nil {
...
func cvFallbackWeightedNoncesScalar(receiver, chunks int, chunkBits uint, nonces []fr.Element) (fr.Element, error) {
	...
	return cvWeightedScalarScalar(nonces[start:start+chunks], chunkBits)
}

func cvFallbackLinkPositionScalar(receiver, chunk, chunks int) int {
	return receiver*chunks + chunk
}

func cvFallbackLinkPointVectorsScalar(proof *cvFallbackLinkProofScalar) [][]bls12381.G1Affine {
...
func cvValidFallbackLinkProofShapeScalar(proof *cvFallbackLinkProofScalar, total, receivers int) bool {
	return cvValidFallbackLinkProofShapeModeScalar(proof, total, receivers, true)
}

func cvValidFallbackLinkProofShapeAfterValidationScalar(proof *cvFallbackLinkProofScalar, total, receivers int) bool {
	return cvValidFallbackLinkProofShapeModeScalar(proof, total, receivers, false)
}

func cvValidFallbackLinkProofShapeModeScalar(
	proof *cvFallbackLinkProofScalar, total, receivers int, validatePoints bool,
) bool {
...
func cvFallbackLinkProofScalarCanonicalBytes(
	proof *cvFallbackLinkProofScalar, context *cvLeafContextScalar, fallbackCount int,
) ([]byte, error) {
	return cvFallbackLinkProofScalarCanonicalBytesMode(proof, context, fallbackCount, true)
}

func cvFallbackLinkProofScalarCanonicalBytesAfterValidation(
	proof *cvFallbackLinkProofScalar, context *cvLeafContextScalar, fallbackCount int,
) ([]byte, error) {
	return cvFallbackLinkProofScalarCanonicalBytesMode(proof, context, fallbackCount, false)
}

func cvFallbackLinkProofScalarCanonicalBytesMode(
	proof *cvFallbackLinkProofScalar, context *cvLeafContextScalar, fallbackCount int, validatePoints bool,
) ([]byte, error) {
	chunks, err := cvChunkCount(context.Profile)
	if err != nil {
		return nil, err
	...
	if fallbackCount <= 0 || !cvValidFallbackLinkProofShapeModeScalar(proof, total, fallbackCount, validatePoints) {
		return nil, fmt.Errorf("invalid CV V2 fallback-link wire")
	}
	var wire bytes.Buffer
	_ = cvWriteBytes(&wire, []byte(cvFallbackLinkWireDomainScalar))
	for _, points := range cvFallbackLinkPointVectorsScalar(proof) {
		if err := cvWritePointVectorMode(&wire, points, validatePoints); err != nil {
			return nil, err
		}
...
func cvDecodeFallbackLinkProofScalar(
	wire []byte, context *cvLeafContextScalar, fallbackCount int,
) (*cvFallbackLinkProofScalar, error) {
	chunks, err := cvChunkCount(context.Profile)
	...
	domain, err := r.bytes(len(cvFallbackLinkWireDomainScalar))
	if err != nil || !bytes.Equal(domain, []byte(cvFallbackLinkWireDomainScalar)) {
	...
	for i, count := range scalarCounts {
		scalars[i], err = cvReadExactScalarVectorScalar(r, count)
		if err != nil {
			return nil, err
		}
	}
	...
	canonical, err := cvFallbackLinkProofScalarCanonicalBytesAfterValidation(proof, context, fallbackCount)
