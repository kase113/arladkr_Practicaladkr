...
func TestCVOldCommitteeKeyBundleScalarUsesIndependentRoleKeys(t *testing.T) {
	...
	for _, role := range []string{cvScalarRoleAPDB, cvScalarRoleControl, cvScalarRoleCoin} {
		info, statErr := os.Stat(cvScalarThresholdSecretPath(secretDir, role, 0))
...
func TestCVOldCommitteeKeyBundleScalarRejectsRoleMutation(t *testing.T) {
	cfg := cvScalarParamsTestConfig()
	params, err := cvDeriveScalarParams(cfg)
	if err != nil {
		t.Fatal(err)
	...
	path := filepath.Join(publicDir, cvOldCommitteeKeyBundleScalarFilename)
	...
	if _, err := cvLoadOldCommitteeKeyBundleScalar(publicDir, secretDir, cfg.SID, uint64(cfg.Epoch), cfg.OldCommittee, []int{0}, params); err == nil {
